    ...
    const fetchBlogData = async () => {
      try {
        ...
        const [postsResponse, categoriesResponse, statsResponse] = await Promise.all([
          fetch(`http://etki360.com/backend/api/blog.php?action=posts&page=${currentPage}&limit=${postsPerPage}&category=${activeCategory !== 'all' ? activeCategory : ''}`),
          fetch('http://etki360.com/backend/api/blog.php?action=categories'),
          fetch('http://etki360.com/backend/api/blog.php?action=stats')
        ]);
